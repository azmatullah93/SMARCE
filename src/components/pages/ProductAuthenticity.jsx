...
const ProductAuthenticity = () => {
  ...
  const handleSubmit = async (scannedQRCode) => {
    ...
    try {
      ...
      setError(null);
      console.log('Calling smart contract with Owner ID:', ownerID, 'and QR Code:', scannedQRCode);
      
      // Call the smart contract using the scanned QR code (string) and ownerID
      const result = await SmartContractService.checkProduct(ownerID, scannedQRCode);
  ...
  return (
    <div className=''>
      ...
      <div className='flex flex-col items-center bg-gray-100 min-h-screen justify-center'>
        <div>
          <h1 className='text-5xl font-spaceGrotesk font-bold mb-20'>Product Authenticity</h1>
          <div>
            <label className='block text-sm font-medium mx-4 text-gray-700' htmlFor='ownerID'>
              Owner ID
            </label>
            <input
        ...
        <div className="mt-4 text-red-600 font-semibold">
          Upon purchasing a product, a receipt containing the transaction hash must be generated. Without this receipt, the product may be considered counterfeit.
        </div>
        ...
        {productInfo ? (
          productInfo.quantity === "0" ? (
            <div className='mt-4 p-4 bg-red-200 rounded'>
              <h2 className='text-xl font-semibold text-red-600'>No Product Left</h2>
            </div>
          ) : (
